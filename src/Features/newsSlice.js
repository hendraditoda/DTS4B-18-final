...
import newsApi, { DEFAULT_PARAMS } from '../apis/newsApi';
...
export const getNews = createAsyncThunk(
  'getNews',
  async ({ category, sort }) => {
    const responseNewsApi = await newsApi.get('/everything', {
      params: {
        ...DEFAULT_PARAMS,
        language: 'en',
        q: category,
...
export const getTopNews = createAsyncThunk(
  'getTopNews',
  async ({ category }) => {
    const responseNewsApi = await newsApi.get('/top-headlines', {
      params: {
        ...DEFAULT_PARAMS,
        category: category,
        pageSize: 10,
